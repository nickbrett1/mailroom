...
agy-dev() {
  ...
  echo "Starting Antigravity with Doppler (common + mailroom)..."
  ...
  doppler run --project common --config dev -- doppler run --forward-signals --project mailroom --config dev -- agy "$@"
}
...
if [[ "$PWD" == "$HOME" ]]; then
  cd /workspaces/mailroom 2>/dev/null
fi
