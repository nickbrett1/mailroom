...
# "mailroom-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "mailroom-dev" is overwritten by
...
AGENT_NAME="mailroom-dev"
WORKSPACE="/workspaces/mailroom"
...
if [ -x "/workspaces/mailroom/scripts/agent-dev.sh" ]; then
  "/workspaces/mailroom/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
