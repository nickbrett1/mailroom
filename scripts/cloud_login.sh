...
if command -v doppler &> /dev/null; then
  if doppler whoami &> /dev/null 2>&1; then
    ...
    if doppler login --no-check-version --yes; then
      ...
      if doppler setup --no-interactive --project mailroom --config dev; then
        echo "✅ Doppler project mailroom/dev configured."
      else
        echo "WARN: doppler setup failed for mailroom/dev - the project may not"
        echo "      exist yet. Create it at https://dashboard.doppler.com, then run:"
        echo "      doppler setup --no-interactive --project mailroom --config dev"
      fi
    else
...
if command -v tailscale &> /dev/null; then
  ...
  if ! sudo tailscale status &> /dev/null; then
    ...
    sudo tailscale up --hostname=mailroom
  else
